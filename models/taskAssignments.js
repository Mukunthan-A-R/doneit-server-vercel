// Kept the exact import file path, pulling pool out directly
...
const removeUserFromTask = async ({ task_id, user_id }) => {
  ...
  try {
    const res = await pool.query(query, [task_id, user_id]);
    if (res.rowCount === 0) {
      return {
        success: false,
        status: 404,
...
const getAssignedUsers = async (taskId) => {
  ...
  try {
    const res = await pool.query(query, [parseInt(taskId)]);
    return {
      success: true,
      status: 200,
      data: res.rows,
